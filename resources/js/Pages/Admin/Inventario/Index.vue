    ...
    <div class="container">
      <div class="admin-page fade-in">
        <div class="admin-header">
          ...
          <Link :href="route('admin.inventario.create')" class="btn btn-primary">+ Registrar Movimiento</Link>
        </div>

        <div class="table-wrap card">
          <table class="admin-table">
            <thead>
              <tr><th>ID</th><th>Producto</th><th>Tipo</th><th>Cantidad</th><th>Técnica</th><th>Fecha</th></tr>
            </thead>
            <tbody>
              <tr v-for="m in movimientos.data" :key="m.id">
                <td>{{ m.id }}</td>
                <td>{{ m.producto?.nombre }}</td>
                <td><span :class="['tipo-badge', m.tipo]">{{ m.tipo }}</span></td>
                <td>{{ m.cantidad }}</td>
                <td>{{ m.tecnica }}</td>
                <td>{{ new Date(m.created_at).toLocaleDateString('es-BO') }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
.tipo-badge.entrada { background:color-mix(in srgb, var(--color-success) 20%, var(--bg-card)); color:var(--color-success); }
